...
class MnistModel(nn.Module):
    def __init__(self, input_size, hidden_size1, hidden_size2, num_classes, device):
        super().__init__()
    ...
    def forward(self, imag):
        ...
        out = F.relu(out)
        out = self.linear3(out)  # вычисляем последний слой
        return out

    def training_step(self, batch):
        images, labels = batch[0].to(self.device), batch[1].to(self.device)
        out = self(images)  # получаем предсказание
...
def main():
    ...
    model = MnistModel(input_size, hidden_size1, hidden_size2, num_classes, device)
    model.to(device)
